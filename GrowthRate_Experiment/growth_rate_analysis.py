...
print("tb194_data shape:", tb194_data.shape)
print("tb194_col_indices:", tb194_col_indices)
...
print("tb204_data shape:", tb204_data.shape)
print("tb204_col_indices:", tb204_col_indices)
...
for conc in sorted_concs:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'plots/TB194_Cm{conc}.pdf')
    plt.close()
    col_start += n_reps
...
for conc in sorted_concs:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'plots/TB204_Cm{conc}.pdf')
    plt.close()
    col_start += n_reps
...
for idx, conc in enumerate(sorted_concs):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'plots/TB194_Cm{conc}_fits.pdf')
    plt.close()
    col_start += n_reps
...
for idx, conc in enumerate(sorted_concs):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'plots/TB204_Cm{conc}_fits.pdf')
    plt.close()
    col_start += n_reps
...
plt.legend()
plt.grid(True)
plt.savefig('plots/MeanGrowthRate_vs_Cm_TB194_TB204.pdf')  # <-- Add this line
plt.show()
